...
public interface SpringDataRatingRepository extends JpaRepository<Rating, RatingKey> {
    ...
    @Query(value = "SELECT EPISODE.ID, AVG(USER_EPISODE_RATING.RATING_VALUE), COUNT(USER_EPISODE_RATING.RATING_VALUE) FROM EPISODE LEFT OUTER JOIN USER_EPISODE_RATING ON EPISODE.ID = USER_EPISODE_RATING.EPISODE_ID GROUP BY EPISODE_ID", nativeQuery = true)
